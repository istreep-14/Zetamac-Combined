  ...
  if (timeRemaining !== null) {
    if (timeRemaining === 0 && gameActive && !sessionSaved) {
      ...
      setTimeout(() => {
        ...
        // Save game session
        console.log(`Saving game session with score: ${score}, duration: ${initialGameDuration}s`);
...
async function refreshAuthTokenWithRefreshToken(refreshToken) {
  try {
    ...
    const { getFirebaseApiKey } = await import('/firebase/firebase-config.js');
...
const GOOGLE_SHEET_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbwwAuBvnvkip5oQvOPguI8FfF3XZk1hwuysplWK1GWCdCC84JYA1zsDBG7YlaypIoUU/exec';

async function sendDataToGoogleSheet(score, problems) {
  try {
...
async function saveSessionToFirestore(score, problems) {
 ...
 url: { stringValue: window.location.href },
    ...
    if (response.ok) {
      ...
      sendDataToGoogleSheet(score, problems);
    } else {
